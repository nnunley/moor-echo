...
mod tests {
    use crate::{
        ast::{BindingPattern, BindingType, EchoAst, LValue},
    ...
    fn test_literals() {
        ...
        // Floats
        assert!(
            matches!(parse("3.14").unwrap(), EchoAst::Float(f) if (f - 3.14).abs() < f64::EPSILON)
        );
    ...
    fn test_identifiers_and_references() {
        ...
        assert!(
            matches!(parse("_underscore").unwrap(), EchoAst::Identifier(s) if s == "_underscore")
        );
