...
/// Target for destructuring assignment
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub enum DestructuringTarget {
    /// Simple variable: x
    Simple(String),
    /// Optional variable with default: ?x = default
    Optional { name: String, default: Box<EchoAst> },
}

impl fmt::Display for DestructuringTarget {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            DestructuringTarget::Simple(name) => write!(f, "{}", name),
            DestructuringTarget::Optional { name, default } => write!(f, "?{} = {}", name, default),
        }
    }
}
...
pub enum EchoAst {
    ...
    Flyweight {
        object: Box<EchoAst>,
        properties: Vec<(String, EchoAst)>, // property name -> value pairs
    },
    ErrorCatch {
        expr: Box<EchoAst>,
        error_patterns: Vec<String>, // E_TYPE, E_PROPNF, etc
        default: Box<EchoAst>,
    },
    MapLiteral {
        entries: Vec<(String, EchoAst)>, // key -> value pairs
    },
    Define {
        name: String,
        value: Box<EchoAst>,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            EchoAst::ErrorCatch { expr, error_patterns, default } => {
                write!(f, "`{} ! ", expr)?;
                for (i, pat) in error_patterns.iter().enumerate() {
                    if i > 0 { write!(f, ", ")?; }
                    write!(f, "{}", pat)?;
                }
                write!(f, " => {}'", default)
            ...
            EchoAst::Define { name, value } => write!(f, "define {} = {}", name, value),
            EchoAst::Spread { expr } => write!(f, "...{}", expr),
