...
impl Evaluator {
    ...
    fn eval_function_call(&mut self, name: &str, args: &[EchoAst], player_id: ObjectId) -> Result<Value> {
        ...
        match name {
            ...
            "ui_add_button" => {
                if arg_values.len() != 3 {
                    return Err(anyhow!("ui_add_button() takes exactly 3 arguments (id, label, action), got {}", arg_values.len()));
                }
            ...
            "ui_add_text" => {
                if arg_values.len() < 2 || arg_values.len() > 3 {
                    return Err(anyhow!("ui_add_text() takes 2-3 arguments (id, text, [style]), got {}", arg_values.len()));
                }
            ...
            "ui_add_div" => {
                if arg_values.len() < 2 || arg_values.len() > 3 {
                    return Err(anyhow!("ui_add_div() takes 2-3 arguments (id, content, [style]), got {}", arg_values.len()));
                }
