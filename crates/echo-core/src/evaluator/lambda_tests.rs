...
use super::*;
use crate::{parser::create_parser, storage::Storage};
...
fn test_simple_lambda_parameters() {
    ...
    assert_eq!(
        result,
        Value::Integer(8),
        "Simple lambda should add parameters"
    );
}
...
fn test_lambda_closure_captures_environment() {
    ...
    // Set up environment
    parser
        .parse("let multiplier = 10")
        .map(|ast| evaluator.eval_with_player(&ast, player_id))
        .unwrap()
        .unwrap();
...
fn test_rest_parameter_in_middle_position() {
    ...
    // For now, our grammar only supports it at the end, but let's test that it
    // works there
    ...
    evaluator
        .eval_with_player(&ast_result.unwrap(), player_id)
        .unwrap();
